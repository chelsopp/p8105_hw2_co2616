...
tidying `Zip Codes` dataset

```{r}
zip_df = 
  read_csv("hw2_data/Zip Codes.csv") %>% 
  janitor::clean_names() %>%
  mutate(file_date = mdy(file_date)) %>% 
  separate(
    file_date, into = c("year", "month", "day")) %>%
  relocate(county, zip_code, year, month, neighborhood) %>% 
  select(-state_fips, -county_fips, -county_code, -day)
```

tidying `zori` dataset

```{r}
zori_df = 
  read_csv("hw2_data/zori.csv") %>% 
  janitor::clean_names()  %>%
  pivot_longer(
  ...
  relocate(county, zip_code, year, month, state, rental_price) %>% 
  select(-metro, -state_name, -region_type, -size_rank, -city, -region_id, -day)
```

joining/merging datasets together

```{r}
zillow_df = 
  left_join(zori_df, zip_df, by = c("county", "zip_code", "year", "month"))
```

The `zillow_df` dataset contains 17284 observations of 7 variables.