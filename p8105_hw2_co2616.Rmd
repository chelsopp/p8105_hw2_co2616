...
zip_df = 
  ...
  relocate(county, zip_code, neighborhood) %>% 
  select(-state_fips, -county_fips, -county_code)
```
...
zori_df = 
  ...
  relocate(county, zip_code, state, rental_price) %>% 
  select(-metro, -state_name, -region_type, -size_rank, -city, -region_id)
```
